...
def load_training_and_test_data():
    ...
    def str_to_img(the_dir, a_list):  # returns list of all images
        ...
        all_images = []
        for el in a_list:
            img = image.load_img(the_dir + "/" + el).resize((128, 128))  # this is a PIL image
    ...
    def augment5(the_dir, savedir):
        ...
        img = image.load_img(the_dir).resize((128, 128))  # this is a PIL image
        x = image.img_to_array(img)  # this is a Numpy array with shape (150, 150, 3)
        # x = x.reshape((1,) + x.shape)  # this is a Numpy array with shape (1, 3, 150, 150)
        x90 = datagen3.apply_transform(x, {'theta': 90})
        x180 = datagen3.apply_transform(x, {'theta': 180})
        x270 = datagen3.apply_transform(x, {'theta': 180})
    ...
    unity_test_images = unity_test_images.reshape((1000, 128, 128, 3))
    ...
    final_list = [X_train, X_test, y_train, y_test]
    return final_list
